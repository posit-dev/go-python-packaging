...
import (
	...
	"github.com/posit-dev/go-python-packaging/distribution/types"
...
func getParserFile(file string, signatureFile string) (ParserData, error) {
	...
	if signatureFile == "" {
		packageFiles, err = distribution.Parse(file)
	} else {
		packageFiles, err = distribution.Parse(file, signatureFile)
	}
