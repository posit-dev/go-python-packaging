...
- `license`: `Types` now derives a license from the legacy free-form `License`
  ...
  what makes it impossible to invent a license here — `3-Clause BSD License`
  tokenizes to `3-Clause` / `BSD` / `License`, and accepting the plausible part
  would attribute `BSD-3-Clause` to a package on the strength of a word.
...
- `license`: SPDX identifiers are now checked against the published SPDX
  License List (`license/data/spdx_licenses.json`, generated by
  `license/data/gen_spdx_licenses.py`) rather than an ad hoc set. Deprecated
  identifiers are excluded, so `GPL-3.0` and `AGPL-3.0` do not resolve — they
  are ambiguous between the `-only` and `-or-later` forms that replaced them.
