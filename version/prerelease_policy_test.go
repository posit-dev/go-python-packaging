...
// Contains is defined as "Filter of one item is non-empty", exactly as
// upstream defines it, so the two can never disagree. Cross-check it.
func TestContainsAgreesWithFilterOfOne(t *testing.T) {
	specs := []string{"", ">=1.0", "<2", "==2.0.*", "!=1.5", "~=1.4", ">=1.0.dev1", "===lolwat"}
	items := []string{"1.0", "1.5", "2.0", "1.5a1", "2.0.dev1", "lolwat", "not a version"}

	for _, spec := range specs {
		for _, policy := range []PreReleases{PreReleasesAuto, PreReleasesInclude, PreReleasesExclude} {
			ss, err := NewSpecifiers(spec, WithPreReleases(policy))
			require.NoError(t, err)
			for _, item := range items {
				want := len(ss.Filter([]string{item})) > 0
				assert.Equal(t, want, ss.Contains(item),
					"spec=%q policy=%s item=%q", spec, policy, item)
			}
		}
	}
}
